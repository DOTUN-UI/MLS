...
(function () {
  ...
  function renderJobsPage() {
    ...
    const syncFromURL = () => {
      if (!form) return;
      ['q', 'city', 'stadium', 'category', 'type', 'status', 'payMin'].forEach((key) => {
    ...
    if (citySel && citySel.options.length <= 1) {
      [...new Set(MW.stadiums.map((s) => s.city))].forEach((c) => {
        citySel.insertAdjacentHTML('beforeend', `<option value="${c}">${c}</option>`);
      });
    }
    if (stadiumSel && stadiumSel.options.length <= 1) {
      MW.stadiums.forEach((s) => {
  ...
  function renderJobPage() {
    ...
    if (!root) return;
    const id = params.get('id') || 'job-gs-pp';
    const job = MW.enrichJob(MW.getJob(id) || MW.jobs[0]);
    const s = job.stadium;
  ...
  function renderApply() {
    ...
    if (!root) return;
    const jobId = params.get('job') || 'job-gs-pp';
    const job = MW.enrichJob(MW.getJob(jobId) || MW.jobs[0]);
    ...
    qs('#next-step')?.addEventListener('click', () => {
      if (step < steps.length - 1) {
        ...
        showToast('Application submitted — redirecting to your dashboard');
        setTimeout(() => (location.href = 'dashboard.html?submitted=1'), 900);
      }
    });
